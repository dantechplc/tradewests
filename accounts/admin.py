...
class InvestmentBenefitInline(admin.TabularInline):
    model = InvestmentBenefits
    extra = 1
@admin.register(Investment)
class InvestmentAdmin(admin.ModelAdmin):
    list_display = (
        "name", "category", "is_active", "min_amount", "max_amount",
        "daily_rate", "duration", "period_in_days",
        "referral_commission", "percentage",
    )
    search_fields = ("name",)
    list_filter = ("duration", "period_in_days")
    inlines = [InvestmentBenefitInline]
    readonly_fields = ()
    fieldsets = (
        ("Basic Info", {
            "fields": ("name", "image", "category", "is_active", "is_promo")
        }),
        ("Investment Limits", {
            "fields": ("min_amount", "max_amount", "percentage")
        }),
        ("Rates & Duration", {
            "fields": ("roi_rate","daily_rate", "duration", "period_in_days", "referral_commission")
        }),
    )
